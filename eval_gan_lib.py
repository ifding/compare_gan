...
def evaluate_tfhub_module(module_spec, eval_tasks, use_tpu,
  ...
  gen_module_spec = os.path.join(module_spec, "gen")
  enc_module_spec = os.path.join(module_spec, "enc")

  #gen_module = hub.Module(gen_module_spec)
          #, name="gen_module")
  #print(gen_module.get_input_info_dict())
  #print(gen_module.get_output_info_dict())
  
  #enc_module = hub.Module(enc_module_spec)
  #print(enc_module.get_input_info_dict())
  #print(enc_module.get_output_info_dict())
  ...
  with tf.Graph().as_default():
    ...
    with tf.Session() as sess:
      ...
      def encode_images():
        """Create encoder for encoding images."""
        images = tf.placeholder(
          shape=[None] + list(dataset.image_shape),
          dtype=tf.float32)
        encoder = hub.Module(enc_module_spec)
        if "labels" in encoder.get_input_info_dict():
          # Conditional GAN.
          assert dataset.num_classes
          labels = tf.random.uniform(
              [batch_size], maxval=dataset.num_classes, dtype=tf.int32)
        ...
        logging.info("Encoder inputs: %s", encoder.get_input_info_dict())
        return images, encoder(inputs=inputs, as_dict=True)["prediction"]

      if use_tpu:
        generated = tf.contrib.tpu.rewrite(sample_from_generator)
      else:
        generated = sample_from_generator()
        images, encoded = encode_images()

      tf.global_variables_initializer().run()

      purity, nmi, ari = eval_utils.cluster_test_dataset(sess, images, 
              encoded, dataset=dataset, num_examples=num_test_examples)
      cluster_result_dict["purity"] = purity
      cluster_result_dict["nmi"] = nmi
      cluster_result_dict["ari"] = ari
  ...
  result_dict = {}
  result_dict.update(cluster_result_dict)
