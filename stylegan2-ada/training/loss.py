...
#----------------------------------------------------------------------------
# Evaluate E and return results as EasyDict.

def eval_E(E, aug, images, labels, report=None, augment_inputs=True, return_aux=0):
    r = dnnlib.EasyDict()
    r.images_aug = images
    r.labels_aug = labels
    if augment_inputs and aug is not None:
    ...
    if aug is not None:
        r.args.augment_strength = aug.get_strength_var()
    scores = E.get_output_for(r.images_aug, r.labels_aug, **r.args)
    r.logits = scores[:, 512:]
    r.labels = tf.nn.softmax(r.logits)
    r.output = scores[:, 0:512]
    return r
...
def stylegan2(G, D, E, aug, fake_labels, real_images, real_labels, r1_gamma=10, pl_minibatch_shrink=2, pl_decay=0.01, pl_weight=2, **_kwargs):
    ...
    E_fake = eval_E(E, aug, G_fake.images, fake_labels, report='fake')
    E_real = eval_E(E, aug, real_images, real_labels, report='real')
    G_rec = eval_G(G, E_real.output, E_real.labels, return_dlatents=True)
    ...
    with tf.name_scope('Loss_main'):
        ...
        G_loss += 0.0 * tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=E_fake.logits, labels=fake_labels))
        G_loss += 0.0 * tf.reduce_mean(tf.square(E_fake.output - fake_latents))
        G_loss += 0.0 * mmd_penalty(E_real.output, fake_latents)
        G_loss += 0.0 * tf.reduce_mean(tf.abs(real_images - G_rec.images))
