...
def D_main(
    ...
    **_kwargs,                          # Ignore unrecognized keyword args.
    ...
    #i Adaptive multiplicative dropout.
    def adrop(x):
...
def E_main(
    ...
    **_kwargs,                          # Ignore unrecognized keyword args.
    ...
    with tf.variable_scope('4x4'):
        ...
        with tf.variable_scope('Dense0'):
            trainable = is_next_layer_trainable()
            x = apply_bias_act(adrop(dense_layer(x, fmaps=nf(0)+label_size, trainable=trainable)), act=act, trainable=trainable)
...
def _E_main(
    ...
    **_kwargs,                          # Ignore unrecognized keyword args.
    ...
    # Inputs.
    images_in.set_shape([None, num_channels, resolution, resolution])
    images_in = tf.transpose(images_in, perm=[0, 2, 3, 1])
    labels_in.set_shape([None, label_size])
    images_in = tf.cast(images_in, dtype)
    labels_in = tf.cast(labels_in, dtype)    
    ...
    with tf.variable_scope('module'):
        trainable = False
        encoder = hub.Module("/home/feid/simclr/simclr_test_ft/hub/9766", trainable=False) # file path
    #print(encoder.get_input_info_dict())
    #print(encoder.get_output_info_dict())
    
    z_dim = 512
    representation = encoder(images_in)
    print(representation)
    
    with tf.variable_scope('Output'):
        output = apply_bias_act(dense_layer(representation, fmaps=z_dim+label_size))
