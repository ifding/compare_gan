...
def training_loop(
    ...
    for gpu, (G_gpu, D_gpu, E_gpu) in enumerate(zip(G_gpus, D_gpus, E_gpus)):
        with tf.name_scope(f'Train_gpu{gpu}'), tf.device(f'/gpu:{gpu}'):
            ...
            terms = dnnlib.util.call_func_by_name(G=G_gpu, D=D_gpu, E=E_gpu, aug=aug, fake_labels=fake_labels, real_images=real_images_var, real_labels=real_labels_var, **loss_args)

            if lazy_regularization:
                #if terms.G_reg is not None: G_reg_opt.register_gradients(tf.reduce_mean(terms.G_reg * G_reg_interval), list(G_gpu.trainables.values()) + list(E_gpu.trainables.values())) 
                if terms.G_reg is not None: G_reg_opt.register_gradients(tf.reduce_mean(terms.G_reg * G_reg_interval), list(G_gpu.trainables.values()))
            ...
            #G_opt.register_gradients(tf.reduce_mean(terms.G_loss), list(G_gpu.trainables.values()) + list(E_gpu.trainables.values()))
            G_opt.register_gradients(tf.reduce_mean(terms.G_loss), list(G_gpu.trainables.values()))
