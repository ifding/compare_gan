...
def create_cifar10(tfrecord_dir, cifar10_dir, ignore_labels):
    ...
    random_labels = np.random.randint(low = 0, high = 10, size = images.shape[0] - test_labels.shape[0])
    labels = np.concatenate([random_labels, test_labels])

    assert ignore_labels in [0, 1]
    assert images.shape == (60000, 3, 32, 32) and images.dtype == np.uint8
    assert labels.shape == (60000,) and labels.dtype in [np.int32, np.int64]
    assert np.min(images) == 0 and np.max(images) == 255
    assert np.min(labels) == 0 and np.max(labels) == 9
    onehot = np.zeros((labels.size, np.max(labels) + 1), dtype=np.float32)
    onehot[np.arange(labels.size), labels] = 1.0
