...
from compare_gan.metrics import clustering_score as clustering_score_lib
import gin.tf
import numpy as np
import six
...
def _run_eval(module_spec, checkpoints, task_manager, run_config,
  ...
  print(module_spec[0].get_tags())
  print(module_spec[1].get_tags())

  if len(module_spec) != 2:
    raise ValueError("module_spec size: {} not supported."
            .format(len(module_spec)))
  ...
  for checkpoint_path in checkpoints:
    ...
    if not tf.gfile.Exists(export_path):
      gen_export_path = os.path.join(export_path, "gen")
      enc_export_path = os.path.join(export_path, "enc")
      module_spec[0].export(gen_export_path, checkpoint_path=checkpoint_path)
      module_spec[1].export(enc_export_path, checkpoint_path=checkpoint_path)
    default_value = -1.0
    try:
